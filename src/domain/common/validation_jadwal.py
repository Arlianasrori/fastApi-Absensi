...
async def validateJadwal(jam_mulai : time,jam_selesai : time,id_kelas : int,hari : HariEnum, id_sekolah : int,id_tahun : int,session : AsyncSession) -> None:
    ...
    findJadwal = (await session.execute(select(Jadwal).where(and_(Jadwal.id_kelas == id_kelas,Jadwal.id_sekolah == id_sekolah,Jadwal.id_tahun == id_tahun,Jadwal.hari == hari)))).scalars().all()
