...
async def getAllAbsen(page : int | None,id_sekolah : int,id_tahun : int,query : FilterAbsenQuery,session : AsyncSession) -> list[AbsenWithSiswaDetail] | ResponseAbsenPag:
    statementGetAbsen = select(Absen).options(joinedload(Absen.siswa),joinedload(Absen.detail)).where(and_(Absen.siswa.and_(Siswa.id_sekolah == id_sekolah),Absen.siswa.and_(Siswa.id_tahun == id_tahun),Absen.id_siswa == query.id_siswa if query.id_siswa else True, Absen.jadwal.and_(Jadwal.hari == query.hari) if query.hari else True, Absen.tanggal >= query.tanggal_mulai if query.tanggal_mulai else True, Absen.tanggal <= query.tanggal_selesai if query.tanggal_selesai else True))
