...
from ..domain.schemas.siswa_schema import SiswaBase, SiswaDetailWithSekolahJumlahSiswa
...
@siswaRouter.get("/profile",response_model=ApiResponse[SiswaDetailWithSekolahJumlahSiswa],tags=["SISWA/AUTH-PROFILE"])
...
async def getAllLaporanAbsen(month : int,year : int | None = None,siswa : dict = Depends(getSiswaAuth),session : sessionDepedency = None) :
